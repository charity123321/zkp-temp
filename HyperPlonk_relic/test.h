...
void test_sum_check_trivial()
{
    ...
    try
    {
        ...
        // 第一项：2*x*y
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term1 = {mle_x, mle_y};
        poly.add_mle_list(term1, Fp(2)); // 系数为2

        // 第二项：3*y*z
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term2 = {mle_y, mle_z};
        poly.add_mle_list(term2, Fp(3)); // 系数为3

        // 第三项：4*x*z
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term3 = {mle_x, mle_z};
...
void test_zero_check_trivial()
{
    ...
    try
    {
        ...
        // 第二项：x*y (负项)
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term2 = {mle_x, mle_y};
        poly.add_mle_list(term2, Fp(-1));

        // 第三项：y*z (正项)
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term3 = {mle_y, mle_z};
        poly.add_mle_list(term3, Fp(1));

        // 第四项：y*z (负项)
        std::vector<shared_ptr<DenseMultilinearExtension<Fp>>> term4 = {mle_y, mle_z};
...
typedef relic_bls12_381_pp P;
typedef P::scalar_field scalar_field;
void test_multilinearKzgPCS(){
    ...
    try{
        ...
        std::vector<scalar_field> point={scalar_field(1),scalar_field(2)};
        ...
        cout << " 测试批量打开" << endl;
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> polynomials = {poly,poly};
        std::vector<std::vector<scalar_field>> points = {point,point};
        std::vector<scalar_field> evals ;
        ...
        // 9. 测试批量验证
        cout << " 测试批量验证" << endl;
        std::vector<Commitment<P>> commitments = {commitment,commitment};
...
void test_multilinearKzgPCS_for_batch(){
    ...
    try{
        ...
        std::vector<scalar_field> evaluations1={
            scalar_field(1),
            scalar_field(4),
            scalar_field(3),
            scalar_field(10)
        ...
        std::vector<scalar_field> evaluations2={
            scalar_field(2),
            scalar_field(5),
            scalar_field(3),
            scalar_field(6)
        ...
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> polynomials = {poly1,poly2};
        ...
        // 9. 测试批量验证
        cout << " 测试批量验证" << endl;
        std::vector<Commitment<P>> commitments = {commitment1,commitment2};
...
void test_productcheck_random(){
    ...
    try
    {
        ...
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> fxs;
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> gxs;

        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> all_poly;
        ...
        aux_info.max_degree = fxs.size()+1;
...
void test_permutationcheck_trivial() {
    ...
    try {
        ...
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> perms=identity_permutation_mles<scalar_field>(num_vars,num_polys);
        reverse(perms.begin(),perms.end());

        
        std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> gxs=fxs;
...
void test_vanilla_plonk_gates() {
    ...
    assert(gate_terms[0].coefficient == 1 && gate_terms[0].selector_index == 0 && 
           gate_terms[0].wire_indices == std::vector<size_t>{0} && "第一项应该是q_L*w1");
    assert(gate_terms[1].coefficient == 1 && gate_terms[1].selector_index == 1 && 
           gate_terms[1].wire_indices == std::vector<size_t>{1} && "第二项应该是q_R*w2");
    assert(gate_terms[2].coefficient == 1 && gate_terms[2].selector_index == 2 && 
           gate_terms[2].wire_indices == std::vector<size_t>{2} && "第三项应该是q_O*w3");
    
    //assert(gate_terms[3].coefficient == 1 && gate_terms[3].selector_index == 3 && 
     //      gate_terms[3].wire_indices == std::vector<size_t>{0,1} && "第四项应该是q_M*w1*w2");
...
void test_hyperplonk_snark(){
    ...
    // 创建选择器 q1 = [1, 1, 1, 1]
    SelectorColumn<scalar_field> q1(std::vector<scalar_field>{
    ...
    // 第一个见证：w1 = [0, 1, 2, 3]
    WitnessColumn<scalar_field> w1(std::vector<scalar_field>{
        scalar_field::zero(),
        scalar_field::one(),
        scalar_field(2),
        scalar_field(3)
    ...
    WitnessColumn<scalar_field> w2(std::vector<scalar_field>{
        scalar_field::zero(),
        scalar_field::one(),
        scalar_field(32),
        scalar_field(243)
