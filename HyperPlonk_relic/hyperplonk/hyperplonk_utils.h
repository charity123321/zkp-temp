...
private:
    ...
    // - w[0] at r_pi
    size_t num_var;
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> polynomials;
    std::vector<Commitment<P>> commitments;
...
std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> build_mle(
    std::vector<row> rows
){
    ...
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> res;
    res.reserve(num_mles);

    for(size_t i=0;i<num_mles;++i){
        std::vector<scalar_field> cur_coeffs;
...
VirtualPolynomial<scalar_field> build_f(
    ...
    size_t num_vars,
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>>& selector_mles,
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>>& witness_mles
){
...
scalar_field eval_f(
    ...
    std::vector<scalar_field>& witness_evals
){
...
scalar_field eval_perm_gate(
    ...
    std::vector<scalar_field>& witness_perm_evals,
