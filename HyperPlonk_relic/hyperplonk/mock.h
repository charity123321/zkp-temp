...
class MockCircuit{
public: 
    std::vector<scalar_field> public_inputs;
    std::vector<WitnessColumn<scalar_field>> witness;
    ...
    MockCircuit(size_t num_contraints,CustomizedGates& gate){
        ...
        */
        ...
        std::vector<SelectorColumn<scalar_field>> selectors(num_selectors);
        std::vector<WitnessColumn<scalar_field>> witnesses(num_witness);

        // 为每个约束生成数据
        for(size_t cs_counter=0;cs_counter<num_contraints;++cs_counter){
            std::vector<scalar_field> cur_selectors;
            std::vector<scalar_field> cur_witness;
        ...
        std::vector<scalar_field> public_inputs_vec;
        ...
        std::vector<scalar_field> permutation=identity_permutation<scalar_field>(merge_nv,1);
