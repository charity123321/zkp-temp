...
public:
    ...
    scalar_field eval_id_oracle(const std::vector<scalar_field>& point){
...
///   - the selector std::vectors
template<typename scalar_field>
class HyperPlonkIndex{
public:
    HyperPlonkParams<scalar_field> params;
    std::vector<scalar_field> permutation;
    std::vector<SelectorColumn<scalar_field>> selectors;

    HyperPlonkIndex()=default;

    HyperPlonkIndex(HyperPlonkParams<scalar_field>& params_,std::vector<scalar_field>& perm_,std::vector<SelectorColumn<scalar_field>>& selectors_):
        params(params_),
        permutation(perm_),
...
struct HyperPlonkProvingKey{
    HyperPlonkParams<scalar_field> params;
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> permutation_oracle;
    std::vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> selector_oracle;
    std::vector<Commitment<P>> selector_commitments;
    std::vector<Commitment<P>> permutation_commitments;
...
struct HyperPlonkVerifyingKey{
    ...
    std::vector<Commitment<P>> selector_commitments;
    std::vector<Commitment<P>> perm_commitments;
};
