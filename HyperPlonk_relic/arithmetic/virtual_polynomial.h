...
public:
    VPAuxInfo aux_info;
    std::vector<pair<F, std::vector<size_t>>> products;
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> flattened_ml_extensions;
        ...
        const F &coefficient)
    {
        ...
        poly.products.emplace_back(coefficient, std::vector<size_t>{0});
    ...
    void add_mle_list(
        const std::vector<shared_ptr<DenseMultilinearExtension<F>>> &mle_list,
        const F &coefficient)
    {
        // 空列表则返回错误
        if (mle_list.empty())
    ...
    F evaluate(const std::vector<F> &point)
    {
        // 变量不一致抛出错误
    ...
    std::vector<F> &buf)
{
    ...
    {
        std::vector<F> sub_r(r.begin() + 1, r.end());
...
template <typename F>
std::vector<F> build_eq_x_r_vec(const std::vector<F> &r)
{
    // we build eq(x,r) from its evaluations
    // we want to evaluate eq(x,r) over x \in {0, 1}^num_vars
    // for example, with num_vars = 4, x is a binary std::vector of 4, then
...
template <typename F>
shared_ptr<DenseMultilinearExtension<F>> build_eq_x_r(const std::vector<F> &r)
{
    ...
    const std::vector<F> &y)
{
    ...
    if (x.size() != y.size())
    {
        cout << "std::vector's length mismatch!" << endl;
        throw;
    }
    F res = F::one();
