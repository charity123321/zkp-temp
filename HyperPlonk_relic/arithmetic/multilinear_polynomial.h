...
// 并返回MLE列表
template <typename F>
std::pair<std::vector<shared_ptr<DenseMultilinearExtension<F>>>, F> random_mle_list(size_t nv, size_t degree)
{
    std::vector<std::vector<F>> multiplicands(degree);
    ...
    // 构建多项式列表
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> list;
    list.reserve(degree);
    for (auto &eval_vec : multiplicands)
    {
...
template <typename F>
std::vector<shared_ptr<DenseMultilinearExtension<F>>> random_zero_mle_list(size_t nv, size_t degree)
{
    std::vector<std::vector<F>> multiplicands(degree);
    for (auto &vec : multiplicands)
    {
...
template <typename F>
std::vector<F> identity_permutation(size_t num_var, size_t num_chunks)
{
...
template <typename F>
std::vector<shared_ptr<DenseMultilinearExtension<F>>> identity_permutation_mles(size_t num_vars, size_t num_chunks)
{
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> res;
...
// 代表随机置换的多项式
template <typename F>
std::vector<F> random_permutation(size_t num_vars, size_t num_chunks)
{
...
template <typename F>
std::vector<shared_ptr<DenseMultilinearExtension<F>>> random_permutation_mles(size_t num_vars, size_t num_chunks)
{
    std::vector<F> s_perm_vec = random_permutation<F>(num_vars, num_chunks);
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> res;
    res.reserve(num_chunks);
    ...
    for (size_t i = 0; i < num_chunks; ++i)
    {
        std::vector<F> chunk_evaluations(
...
DenseMultilinearExtension<F> fix_variables_no_par(
    ...
    const std::vector<F> &partial_point)
{
    assert(partial_point.size() <= poly.num_vars() &&
           "invalid size of partial point");

    size_t nv = poly.num_vars();
    std::vector<F> poly_evals = poly.get_evaluations();
...
shared_ptr<DenseMultilinearExtension<F>> merge_polynomial(
    const std::vector<shared_ptr<DenseMultilinearExtension<F>>> &polynomials)
{
