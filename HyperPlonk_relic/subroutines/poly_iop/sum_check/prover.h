...
// 计算 w_j = 1/prod_{i\neq j}(x_i-x_j)
template <typename F>
std::vector<F> barycenter_weights(const std::vector<F> &points)
{
...
public:
    ...
    SumCheckProver prover_init(const VirtualPolynomial<F> &polynomial)
    {
        ...
        for (size_t degree = 1; degree < polynomial.aux_info.max_degree; ++degree)
        {
            ...
            std::vector<F> weights = barycenter_weights<F>(points);
    ...
    ProverMessage prover_round_and_update_state(const optional<F> &challenge)
    {
        ...
        std::vector<DenseMultilinearExtension<F>> flattened_ml_extensions;
        ...
        if (challenge.has_value())
        {
            ...
            {
                mle = fix_variables_no_par<F>(mle, std::vector<F>{r});
            }
        }
        ...
        std::vector<F> products_sum(state_.poly.aux_info.max_degree + 1, F::zero());
