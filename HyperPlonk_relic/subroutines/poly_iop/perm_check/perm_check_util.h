...
template <typename F>
std::pair<std::vector<shared_ptr<DenseMultilinearExtension<F>>>,
     std::vector<shared_ptr<DenseMultilinearExtension<F>>>>
computer_nums_and_denoms(
    ...
    const std::vector<shared_ptr<DenseMultilinearExtension<F>>> &perms)
{
    size_t num_vars = (*fxs[0]).num_vars();
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> numerators;
    std::vector<shared_ptr<DenseMultilinearExtension<F>>> denominators;

    std::vector<shared_ptr<DenseMultilinearExtension<F>>> s_id = identity_permutation_mles<F>(num_vars, fxs.size());

    for (size_t l = 0; l < fxs.size(); ++l)
    {
        std::vector<F> numerator_evals;
        std::vector<F> denominator_evals;
