...
template <typename F>
std::vector<DenseMultilinearExtension<F>> eq_extension(const std::vector<F> &t)
{
    size_t dim = t.size();
    std::vector<DenseMultilinearExtension<F>> result;
    result.reserve(dim);

    for (size_t i = 0; i < dim; ++i)
    {
        F ti = t[i];
        std::vector<F> poly(1 << dim);
...
template <typename F>
// poly.size()=2^k
std::vector<F> remove_dummy_variable(const std::vector<F> &poly, size_t pad)
{
    if (pad == 0)
    {
        return poly;
...
struct MultilinearProverParam
{
    ...
    std::vector<Evaluations<G1<P>>> power_of_g;
    /// generator for G1
    G1<P> g;
    /// generator for G2
    G2<P> h;
...
public:
    ...
    pair<ProverParam, VerifierParam> trim(size_t supported_num_vars)
    {
        ...
        {
            cout << "SRS does not support target number of vars." << endl;
            throw;
        }
    ...
    StructuredReferenceString get_srs_for_test(size_t num_vars)
    {
        ...
        std::vector<DenseMultilinearExtension<scalar_field>> eq_polys = eq_extension<scalar_field>(t);

        list<DenseMultilinearExtension<scalar_field>> eq_list(eq_polys.begin(), eq_polys.end());
        list<std::vector<scalar_field>> eq_arr;
        std::vector<scalar_field> base = eq_list.back().get_evaluations();
        eq_list.pop_back();
        ...
        for (size_t i = num_vars; i > 0; --i)
        {
            ...
            if (idx != 0)
            {
                std::vector<scalar_field> mul = eq_list.back().get_evaluations();
                eq_list.pop_back();

                std::vector<scalar_field> new_base;
        ...
        std::vector<scalar_field> pp_powers;
        size_t total_scalar = 0;

        for (size_t i = 0; i < num_vars; ++i)
        {
            std::vector<scalar_field> eq = eq_arr.front();
            eq_arr.pop_front();
        ...
        */
        // ==============================================================
    // 替换 libff::batch_exp 为 RELIC + OpenMP 多线程加速
    // ==============================================================
    std::vector<typename P::G1_type> pp_g(total_scalar);

    // 启动 OpenMP 并行计算
    #pragma omp parallel for
    for (size_t i = 0; i < total_scalar; ++i) {
        // 直接复用我们之前重载的乘法运算符
        pp_g[i] = g * pp_powers[i]; 
        
        // 【关键】将生成的点归一化为仿射坐标 (Affine)
        // 这是因为你后续要用 AVX-512 做 MSM 加速，底层的 base 必须是仿射坐标
        ep_norm(pp_g[i].value, pp_g[i].value); 
    }
    // ==============================================================
        ...
        */
        ...
        for (size_t i = 0; i < num_vars; ++i)
        {
            ...
            // 验证正确性
            std::vector<scalar_field> zero_vec(num_vars - i, scalar_field::zero());
            scalar_field t_eval_0 = eq_eval<scalar_field>(zero_vec, std::vector<scalar_field>(t.begin() + i, t.end()));
        ...
        */
       // ==============================================================
        // 替换 libff G2 批处理为 RELIC 并行计算
        // ==============================================================
        std::vector<typename P::G2_type> h_mask(num_vars);

        #pragma omp parallel for
        for (size_t i = 0; i < num_vars; ++i) {
            h_mask[i] = h * t[i];
            
            // G2 也必须归一化为仿射坐标，供 Verifier 的 Pairing 使用
            ep2_norm(h_mask[i].value, h_mask[i].value); 
        }
        // ==============================================================
        // 构建通用参数
...
{
    cout << " h_mask: " << endl;
    std::vector<G2<P>> h_mask = srs.universal_params().h_mask;
    for (auto &h : h_mask)
    {
