...
std::pair<MultilinearKzgProof<P>, scalar_field> open_internal(
    const MultilinearProverParam<P> &prover_param,
    DenseMultilinearExtension<scalar_field> &polynomial,
    const std::vector<scalar_field> &point)
{

    if (polynomial.num_vars() > prover_param.num_vars)
    {
        cout << "Polynomial num_vars exceed the limit." << endl;
        throw;
    }
    if (polynomial.num_vars() != point.size())
    {
        cout << "Polynomial num_vars does not match point len." << endl;
        throw;
    }

    size_t nv = polynomial.num_vars();
    // the first `ignored` SRS std::vectors are unused for opening.
    size_t ignored = prover_param.num_vars - nv + 1;
    std::vector<scalar_field> f = polynomial.get_evaluations();
    ...
    for (size_t i = 0; i < nv; ++i)
    {
        ...
        size_t k = nv - 1 - i;
        size_t cur_dim = 1 << k;
        std::vector<scalar_field> q(cur_dim, scalar_field::zero());
        std::vector<scalar_field> r(cur_dim, scalar_field::zero());
        ...
        */
        G1<P> proof_point;
        ep_set_infty(proof_point.value);
        if(cur_dim>0){
            int WBITS = (cur_dim>1024)?16:12;

            const bn_t* scalars_ptr = reinterpret_cast<const bn_t*>(q.data());
            const ep_t* points_ptr  = reinterpret_cast<const ep_t*>(gi.evals.data());

            PIP_SIMD::pip_threads(scalars_ptr,points_ptr,cur_dim,proof_point.value,WBITS);

            ep_norm(proof_point.value,proof_point.value);
        }
    ...
    const MultilinearKzgProof<P> &proof)
{
    ...
    std::vector<G2<P>> h_mul = libff::batch_exp<G2<P>, scalar_field>(scalar_size, window_size, h_table, point);
    */
   // ==============================================================
    // 替换 libff G2 的 batch_exp 为 RELIC 并行标量乘法
    // ==============================================================
    std::vector<typename P::G2_type> h_mul(num_var);

    #pragma omp parallel for
    for (size_t i = 0; i < num_var; ++i) {
        // 直接在 G2 上做标量乘法
        h_mul[i] = verifier_param.h * point[i]; 
        
        // 【极度关键】验证阶段马上要进行 Pairing（双线性配对）
        // RELIC 的 Pairing 强依赖于输入的点必须是仿射坐标 (Z=1)
        // 所以这里必须调用 ep2_norm！
        ep2_norm(h_mul[i].value, h_mul[i].value); 
    }
    // ==============================================================
    size_t ignored = verifier_param.num_vars - num_var;

    // Compute h_vec[i]=h_mask[i]-h_mul[i]
    std::vector<G2<P>> h_vec;
    ...
    std::vector<pair<G1_precomp<P>, G2_precomp<P>>> pairings;
    ...
    std::vector<G1_precomp<P>> g1_ele;
    std::vector<G2_precomp<P>> g2_ele;
    ...
    */
   // 1. 准备批量配对点
    std::vector<typename P::G1_type> g1_ele;
    std::vector<typename P::G2_type> g2_ele;
    g1_ele.reserve(num_var + 1);
    g2_ele.reserve(num_var + 1);

    for (size_t i = 0; i < num_var; ++i) {
        g1_ele.push_back(proof.proofs[i]);
        g2_ele.push_back(h_vec[i]);
    }

    // 2. 最后一项
    typename P::G1_type g1_last = (verifier_param.g * value) - commitment.point;
    ep_norm(g1_last.value, g1_last.value); // 必须归一化
    g1_ele.push_back(g1_last);
    g2_ele.push_back(verifier_param.h);

    // 3. 执行同时配对 (Simultaneous Pairing)
    typename P::GT_type res = P::multi_pairing(g1_ele, g2_ele);
    return (res == P::GT_type::one());
...
public:
        ...
        const shared_ptr<DenseMultilinearExtension<scalar_field>> &poly)
    {
        ...
        {
            cout << "MLE length exceeds param limit." << endl;
        }

        size_t ignored = prover_param.num_vars - (*poly).num_vars();
        std::vector<scalar_field> scalars = (*poly).get_evaluations();
        /*
        G1<P> commitment_point = libff::multi_exp<G1<P>, scalar_field, multi_exp_method_BDLO12>(
            ...
            1);
        */
       G1<P> commitment_point;
       ep_set_infty(commitment_point.value);
        ...
        if (num_elements > 0) {
            ...
            const ep_t* points_ptr  = reinterpret_cast<const ep_t*>(prover_param.power_of_g[ignored].evals.data());
            
            PIP_SIMD::pip_threads(scalars_ptr, points_ptr, num_elements, commitment_point.value, WBITS);
            
            // 正则化
            ep_norm(commitment_point.value, commitment_point.value);
        }
    ...
    bool batch_verify(
        const MultilinearVerifierParam<P> &verifier_param,
        const std::vector<Commitment<P>> &commitments,
        const std::vector<std::vector<scalar_field>> &points,
