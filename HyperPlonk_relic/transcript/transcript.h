...
private:
    ...
    std::vector<uint8_t> string_to_bytes(const string &str)
    {
    ...
    template <typename T>
    std::vector<uint8_t> to_bytes([[maybe_unused]] const T &obj)
    {
...
public:
    ...
    void append_message(const string &label, const std::vector<uint8_t> &msg)
    {
        // 添加标签长度和标签
    ...
    template <typename F>
    std::vector<F> get_and_append_challenge_vector(const string &label, size_t len)
    {
    ...
    const std::vector<uint8_t> &get_message_buffer() const
    {
