...
public:
    ...
    DenseMultilinearExtension(size_t num_vars, const std::vector<F> &evaluations)
        : num_vars_(num_vars), evaluations_(evaluations) {}

    size_t num_vars() const { return num_vars_; }

    const std::vector<F> &get_evaluations() const { return evaluations_; }
    ...
    DenseMultilinearExtension<F> rand(size_t num_var)
    {
        ...
        for (size_t i = 0; i < (1ULL << num_var); ++i)
        {
            ...
            evals.push_back(F::random_element());
        }
    ...
    /*
        DenseMultilinearExtension<F> from_evaluations_slice(
            ...
            return from_evaluations_vec(num_vars,std::vector(evaluations));
        }
    */
        ...
        size_t num_vars, const F *evaluations, size_t evaluations_size)
    {
        return from_evaluations_vec(
            num_vars,
            std::vector<F>(evaluations, evaluations + evaluations_size));
    }

    DenseMultilinearExtension<F> from_evaluations_vec(
        size_t num_vars, std::vector<F> evaluations)
    {
    ...
    */
    ...
    DenseMultilinearExtension<F> fix_variables(const std::vector<F> &partial_point)
    {
        ...
        return DenseMultilinearExtension<F>(
            nv - dim,
            std::vector<F>(poly.begin(), poly.begin() + (1ULL << (nv - dim))));
    }
    // 固定所有变量
