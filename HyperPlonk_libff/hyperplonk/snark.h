...
public: 
    pair<HyperPlonkProvingKey<P,scalar_field,PCS>,HyperPlonkVerifyingKey<P,scalar_field,PCS>> 
    preprocess(
    ...
    prove(
        ...
        const vector<scalar_field>& pub_input,
        const vector<WitnessColumn<scalar_field>>& witnesses
    );

    bool verify(
        HyperPlonkVerifyingKey<P,scalar_field,PCS>& vk,
        vector<scalar_field>& pub_input,
...
HyperPlonkSNARK<P,scalar_field,PCS>::preprocess(
    ...
    vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> permutation_oracles;
    vector<Commitment<P>> perm_comms;
    size_t chunk_size=1ULL<<num_vars;

    vector<scalar_field> tmp;
    ...
    vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> selector_oracles;
    ...
    return make_pair(move(proving_key),move(verifying_key));
}
...
HyperPlonkSNARK<P,scalar_field,PCS>::prove(
    ...
    // transcript
    // ======================================================================= 
    vector<shared_ptr<DenseMultilinearExtension<scalar_field>>> witness_polys;
    ...
    vector<scalar_field> perm_check_point=perm_check_proof.zero_check_proof.point;
    // =======================================================================
    // 4. Generate evaluations and corresponding proofs
    // - permcheck
    ...
    // (perm_check_point[2..n], 0)
    vector<scalar_field> perm_check_point_0;
    ...
    // (perm_check_point[2..n], 1)
    vector<scalar_field> perm_check_point_1;
    ...
    // (1, ..., 1, 0)
    vector<scalar_field> prod_final_query_point(num_vars,scalar_field::one());
    ...
    // - 4.4. public input consistency checks
    //   - pi_poly(r_pi) where r_pi is sampled from transcript
    vector<scalar_field> r_pi=transcript.get_and_append_challenge_vector<scalar_field>("r_pi",ell);
    vector<scalar_field> r_pi_padded=r_pi;
    vector<scalar_field> zeros(num_vars-ell,scalar_field::zero());
...
HyperPlonkSNARK<P,scalar_field,PCS>:: verify(
    ...
    vector<scalar_field> witness_prem_evals(
        proof.batch_openings.f_i_eval_at_point_i.begin()+7+num_witnesses,
        proof.batch_openings.f_i_eval_at_point_i.begin()+7+2*num_witnesses
    );

    vector<scalar_field> witness_gate_evals(
        proof.batch_openings.f_i_eval_at_point_i.begin()+7+2*num_witnesses,
    ...
    vector<scalar_field> zero_check_point=zero_check_sub_claim.point;
    ...
    vector<scalar_field> perm_check_point=perm_check_sub_claim.product_check_sub_claim.zero_check_sub_claim.point;
    ...
    for(size_t i=0;i<num_witnesses;++i){
        vector<scalar_field> ith_point=gen_eval_point<scalar_field>(i,log2(num_witnesses),perm_check_point);
